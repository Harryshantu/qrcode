...
import com.theektek.qrcode.archcomp.SingleLiveEvent
import com.theektek.qrcode.mediaresource.EntityMediaResource
import com.theektek.qrcode.repository.OperationResult.Operation
 ...
 * @param aPI_COSTCENTER_SRV_Entities OData service
...
class Repository<T : EntityValue>(
        private val aPI_COSTCENTER_SRV_Entities: API_COSTCENTER_SRV_Entities,
    ...
    init {
        ...
        if (EntityMediaResource.isV4(aPI_COSTCENTER_SRV_Entities.metadata.versionCode) && EntityMediaResource.hasMediaResources(entitySet)) {
    ...
    fun read() {
        ...
        aPI_COSTCENTER_SRV_Entities.executeQueryAsync(dataQuery,
            { result ->
    ...
    fun read(parent: EntityValue, navigationPropertyName: String) {
        ...
        aPI_COSTCENTER_SRV_Entities.loadPropertyAsync(navigationProperty, parent, dataQuery,
            {
    ...
    fun create(newEntity: T, media: StreamBase) {
        if (newEntity.entityType.isMedia) {
            aPI_COSTCENTER_SRV_Entities.createMediaAsync(newEntity, media,
                {
    ...
    fun create(newEntity: T) {
        ...
        aPI_COSTCENTER_SRV_Entities.createEntityAsync(newEntity,
            {
                insertToCache(newEntity, entities)
                if (relatedEntities.size > 0) {
                    insertToCache(newEntity, relatedEntities)
    ...
    fun update(updateEntity: T) {
        aPI_COSTCENTER_SRV_Entities.updateEntityAsync(updateEntity,
            {
    ...
    fun delete(deleteEntities: MutableList<T>) {
        ...
        aPI_COSTCENTER_SRV_Entities.applyChangesAsync(deleteChangeSet,
            {
