...
import com.theektek.qrcode.databinding.ActivityEntityitemBinding
 ...
 * - [ACostCenterTextCreateFragment]: to create or edit a(n) ACostCenterTextType,
 * - [ACostCenterTextDetailFragment]: to display the details of a(n) ACostCenterTextType,
 * - [ACostCenterTextListFragment]: to list ACostCenterText.
 *
 ...
 * what to do.
 */
class ACostCenterTextActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<ACostCenterTextType> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ACostCenterTextListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: ACostCenterTextType?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ACostCenterTextCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: ACostCenterTextType?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, ACostCenterTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, ACostCenterTextDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = ACostCenterTextCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: ACostCenterTextType?) {
        val updateFragment = ACostCenterTextCreateFragment()
        val arguments = Bundle()
    ...
    internal class ConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                (fragmentActivity as ACostCenterTextActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
            }
    ...
    internal class DeleteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val fragmentActivity = requireActivity() as AppCompatActivity
            val builder = MaterialAlertDialogBuilder(fragmentActivity, R.style.AlertDialogStyle)
            val viewModel = ViewModelProvider(fragmentActivity)[ACostCenterTextTypeViewModel::class.java]
